/*
  Adapted from Generic_RGB_CCT and v1 fastled Arduino
  Uses fastLED and 2 PWM channels to control for white temperatures
*/
#include <FS.h>
...
state light;
bool inTransition, entertainmentRun, useDhcp = true; // change usedhcp for a static address
byte mac[6], packetBuffer[8];
unsigned long lastEPMillis;

//settings
char *lightName = "New Hue RGB-CCT SM16726 light";
char *colorOrder = "RGB";
uint8_t scene = 0, startup = false, onPin = 1, offPin = 3, pins[] = {4, 5, 14, 4}; //cold white, warm white, data pin, clock pin
bool hwSwitch = false;
...
bool loadConfig() {
  ...
  strcpy(colorOrder, json["order"]);
  pins[0] = (uint8_t) json["cw"];
  pins[1] = (uint8_t) json["ww"];
  pins[2] = (uint8_t) json["data"];
  pins[3] = (uint8_t) json["clock"];
...
void setup() {
  ...
  server.on("/config", []() {
    ...
    root["order"] = colorOrder;
    root["cw"] = pins[0];
    root["ww"] = pins[1];
    root["datapin"] = pins[2];
    root["clockpin"] = pins[3];
    root["hw"] = hwSwitch;
    root["on"] = onPin;
    root["off"] = offPin;
  ...
  server.on("/", []() {
    if (server.hasArg("scene")) {
      ...
      server.arg("order").toCharArray(colorOrder, server.arg("order").length() + 1);
      pins[0] = server.arg("cw").toInt();
      pins[1] = server.arg("ww").toInt();
      pins[2] = server.arg("datapin").toInt();
      pins[3] = server.arg("clockpin").toInt();
